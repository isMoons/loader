...
-- Tambahkan kode custom dropdown DI SINI
    function Fluent:CreateCustomDropdown(tab, options)
    local Section = tab:CreateSection(options.Title or "Custom Dropdown")

    -- Container utama dropdown
    local Container = Section:AddGroupbox(options.Title or "Dropdown")

    -- Input untuk pencarian/filter
    local SearchBox = Container:AddInput("CustomDropdownSearch", {
        Title = options.SearchTitle or "Search...",
        Placeholder = options.SearchPlaceholder or "Type to filter...",
        Numeric = false,
        Finished = false
    })

    -- Dropdown utama
    local Dropdown = Container:AddDropdown("CustomDropdown", {
        Title = options.DropdownTitle or "Select an Option",
        Values = options.Values or {},
        Multi = options.Multi or false,
        Default = options.Default or nil
    })

    -- Fungsi untuk filter hasil berdasarkan input pencarian
    SearchBox:OnChanged(function(value)
        local filteredValues = {}
        for _, item in ipairs(options.Values) do
            if item:lower():find(value:lower()) then
    ...
    if options.Callback then
        Dropdown:OnChanged(options.Callback)
    end
    ...
    local PlayerGui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
        ...
        Tabs.Main:AddToggle("autoEquip", {
            ...
            Description = "Doesn't work with Instant Reel",
            Default = false,
...
-- Ambil semua zona memancing dan masukkan ke dalam tabel
for _, v in ipairs(FishingZonesFolder:GetChildren()) do
    if not table.find(FishingSpots, v.Name) then
        table.insert(FishingSpots, v.Name)
    end
end
...
local floatHeight = 20 -- Ketinggian default saat mengambang

-- Tambahkan input untuk mengatur ketinggian karakter
...
-- Fungsi untuk teleport dan mengambang
local function FloatAboveZone(zoneName)
    ...
    -- Aktifkan auto teleportasi
    autoTeleportEnabled = true

    -- Tunggu hingga teleportasi dinonaktifkan
    while autoTeleportEnabled do
        task.wait(1)
    end

    -- Hapus efek anti-gravity
    BodyVelocity:Destroy()

    -- Kembalikan kecepatan gerakan karakter
    Humanoid.WalkSpeed = WalkSpeedBackup
    Humanoid.JumpPower = JumpPowerBackup

    -- Kembali ke posisi awal jika ada
    if originalCFrame then
        HumanoidRootPart.CFrame = originalCFrame
    end
end

-- Teleportasi berdasarkan dropdown
