...
local DesiredName = Desired and Desired.Name

if not Desired then return end

local enabled = true

-- Menambahkan tombol menggunakan Tabs.Items
Tabs.Exclusives:AddButton({
    Title = "Toggle Auto Appraise",
    Description = "Enable or disable auto appraise.",
    Callback = function()
        enabled = not enabled
    end
})
...
RunService.RenderStepped:Connect(function()
    ...
    local Tool = Character:FindFirstChild(DesiredName) or LocalPlayer.Backpack:FindFirstChild(DesiredName)
    if Tool then Tool.Parent = Character end
end)

local LastAppraise

-- Fungsi untuk melakukan appraise dengan tombol
local function startAppraising()
