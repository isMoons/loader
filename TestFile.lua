...
if not isfile('Ancestral.png') then
    writefile("Ancestral.png", game:HttpGet('https://github.com/isMoons/loader/blob/main/Ancestral.png'))
end
...
local LRM_ScriptVersion = "v0001" -- Debugging Purpose Only
...
local function getLunorIcon()
    ...
    local success, product = pcall(function()
        return getcustomasset(readfile('Ancestral.png'))
    end)

    if not success or identifyexecutor():find("Cryptic") then
        asset = "http://www.roblox.com/asset/?id=81972108244476"
    else
        asset = product
    end
    return asset
...
local main = lib:Load({
    Title = 'Fisch '..formatVersion(LRM_ScriptVersion)..' | ' .. gradient("discord.gg/ancestral").. " | ".. RoleChecker(),
...
[+] Keyless System (Lunor LOL)
[+] Credit @Fearless
[+] I don't know anymore lol skid
]])
...
if _G.offsetFrame then
    ...
    if _G.undoFrame and _G.undoFrame[_G.userId] then
        ...
        lockTextSafe(_G.userFrame:WaitForChild("NameFrame"):WaitForChild("BGFrame"):WaitForChild("OverlayFrame"):WaitForChild("PlayerName"):WaitForChild("PlayerName"), "⭐ Ancestral!")
...
function StatusEmbed()
    function NoDiddy()
        if HiddenMode then
        return "Ancestral User <3"
        else
        return "@"..LocalPlayer.DisplayName.."("..LocalPlayer.Name..")"
        end
    end
    return {
    ["title"] = tostring(NoDiddy()).." - Ancestral // Fisch",
    ["description"] = "◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤◢◤\n"
    .. "**« Account Info »**\n"
...
local function onValueChanged(value)
    ...
    local embed = createWebhookEmbed(
        "Ancestral Auto Catcher Notifier",
        description,
