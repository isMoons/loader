...
-- AutoCast End
    ...
    local section = Tabs.CharacterTab:AddSection("Feature Character")
...
local function smoothTeleport(target)
    if HumanoidRootPart and target then
        ...
        if humanoid then
            ...
            local duration = math.clamp(distance / 50, 1, 5) 

            local tweenInfo = TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(HumanoidRootPart, tweenInfo, {CFrame = target.CFrame + Vector3.new(0, 5, 0)})
